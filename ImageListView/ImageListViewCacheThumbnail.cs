...
namespace Manina.Windows.Forms
{
    ...
    /// item thumbnails.
    /// </summary>
    public class ImageListViewCacheThumbnail : IDisposable
    {
        ...
        public static PersistentCache DiskCache = new PersistentCache();
        ...
        /// </summary>
        public void Clear( bool cleanDiskCache = false)
        {
            foreach (CacheItem item in thumbCache.Values)
                item.Dispose();
            thumbCache.Clear();
        ...
        /// <param name="autoRotate">AutoRotate property of the owner control.</param>
        /// <param name="useWIC">Whether to use WIC.</param>
        public void AddToGalleryCache(Guid guid, ImageListViewItemAdaptor adaptor, object key, Size thumbSize, UseEmbeddedThumbnails useEmbeddedThumbnails, bool autoRotate, bool useWIC)
        {
            // Already cached?
        ...
        {
            if (thumbCache.TryGetValue(guid, out CacheItem item) && item != null && item.Size == thumbSize && item.UseEmbeddedThumbnails == useEmbeddedThumbnails && item.AutoRotate == autoRotate && item.UseWIC == useWIC)
                return item.State;
        ...
        private void RunWorker(CacheRequest item, int priority)
        {
            ...
            if (item.RequestType == RequestType.Thumbnail)
            {
                ...
                {
                    bool value = processing[item.Guid];
                    processing.Remove(item.Guid);
                    processing.Add(item.Guid, value);
                }
                else
