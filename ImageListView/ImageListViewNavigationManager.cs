...
namespace Manina.Windows.Forms
{
    ...
    internal class ImageListViewNavigationManager : IDisposable
    {
        ...
        public static bool addcheck = false;
        ...
        public void MouseMove(MouseEventArgs e)
        {
            ...
            else if (ResizingPane)
            {
                ...
                int width = mImageListView.PaneWidth + delta;
                if (width > 2)
                ...
                mImageListView.PaneWidth = width;
                ...
                Math.Abs(e.Location.Y - lastMouseDownLocation.Y) > SelectionTolerance)))
            {
                ...
                else if (lastMouseDownOverItem && HoveredItem != null && mImageListView.AllowDrag)
                {
                    ...
                    foreach (ImageListViewItem item in mImageListView.SelectedItems.Union(mImageListView.CheckedItems))
                    {
                        string key = item.Tag?.ToString() ?? item.Text;
                        if (!filenames.ContainsKey(key))
                            filenames.Add(key, item.FileName);
                    }
        ...
        public void MouseUp(MouseEventArgs e)
        {
            ...
            {
                ResizingPane = false;
                mImageListView.OnPaneResized(new PaneResizedEventArgs(mImageListView.PaneWidth));
            }
            else if (MouseSelecting)
            {
            ...
            else if (lastMouseDownInItemArea && lastMouseDownOverItem && HoveredItem != null && LeftButton)
            {
                ...
                else if (ShiftKey)
                {
                    ...
                    if (mImageListView.ScrollOrientation == ScrollOrientation.VerticalScroll)
                    {
                        ...
                        for (int row = startRow; row <= endRow; row++)
                        {
                            for (int col = (row == startRow ? startCol : 0); col <= (row == endRow ? endCol : mImageListView.layoutManager.Cols); col++)
                            {
                ...
                mImageListView.AllowColumnClick && HoveredColumn != null && HoveredSeparator == null)
            {
                if (!suppressClick)
                {
                    if (LeftButton)
                    {
                        ...
                        {
                            mImageListView.SortColumn = mImageListView.Columns.IndexOf(HoveredColumn);
                            mImageListView.SortOrder = SortOrder.Ascending;
                            mImageListView.Sort();
                        }
                    }
        ...
        public void DragDrop(DragEventArgs e)
        {
            ...
            else
            {
                ...
                Dictionary<string, string> filenames = (Dictionary<string, string>)e.Data.GetData(DataFormats.FileDrop);
        ...
        private void ScrollTimer_Tick(object sender, EventArgs e)
        {
            ...
            mImageListView.OnMouseMoveUsed(new MouseEventArgs(Control.MouseButtons, 0, location.X, location.Y, 0));
            mImageListView.OnMouseWheelUsed(new MouseEventArgs(MouseButtons.None, 0, location.X, location.Y, delta));
        }
        #endregion
    }
}