...
namespace Manina.Windows.Forms
{
    ...
    public static partial class ImageListViewRenderers
    {
                ...
                lock (lockObject)
                {
                    ...
                    if (currentGeneration == flakeGeneration)
                    {
                        ...
                        if (flakes.Count < maxFlakeCount)
                        {
                            ...
                            {
                                Rotation = 360.0 * random.NextDouble(),
                                Location = new Point(random.Next(displayBounds.Left, displayBounds.Right), -20)
                            };
        ...
        public class NoirRenderer : ImageListViewRenderer
        {
            ...
            public override void DrawColumnHeader(Graphics g, ImageListViewColumnHeader column, ColumnState state, Rectangle bounds)
            {
                // Paint background
                    ...
                    using (StringFormat sf = new StringFormat())
                    {
                        ...
                        using (Brush brush = new SolidBrush(Color.White))
                        {
                            g.DrawString(column.Text,
                                (ImageListView.ColumnHeaderFont ?? ImageListView.Font),
                                brush, bounds, sf);
                        }
                    }
                }
            ...
            public override void DrawColumnHeader(Graphics g, ImageListViewColumnHeader column, ColumnState state, Rectangle bounds)
            {
                ...
                if (bounds.Width > 4)
                {
                    using (StringFormat sf = new StringFormat())
                    {
                        ...
                        sf.LineAlignment = StringAlignment.Center;
                        sf.Trimming = StringTrimming.EllipsisCharacter;
                        g.DrawString(column.Text, (ImageListView.ColumnHeaderFont ?? ImageListView.Font),
            ...
            public override void DrawGroupHeader(Graphics g, string name, Rectangle bounds)
            {
                ...
                if (bounds.Width > 4)
                {
                    using (StringFormat sf = new StringFormat())
                    {
                        ...
                        sf.LineAlignment = StringAlignment.Center;
                        sf.Trimming = StringTrimming.EllipsisCharacter;
                        g.DrawString(name, (ImageListView.GroupHeaderFont ?? ImageListView.Font),
                            SystemBrushes.WindowText, bounds, sf);
                    }
                }
            }
        ...
        public class MeerkatRenderer : ImageListViewRenderer
        {
            ...
            public override Size MeasureItem(View view)
            {
                ...
                else
                {
                    ...
                    Size itemSize = new Size
                    {
                        Height = ImageListView.ThumbnailSize.Height + textHeight + 4 * 3,
                        Width = ImageListView.ThumbnailSize.Width + 2 * 3
                    };
