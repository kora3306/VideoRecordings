...
namespace Manina.Windows.Forms
{
    ...
    public static class Utility
    {
        ...
        {
            float f = Math.Max((float)image.Width / (float)fit.Width, (float)image.Height / (float)fit.Height);
            if (f < 1.0f) f = 1.0f; // Do not upsize small images
            int width = (int)Math.Round((float)image.Width / f);
            int height = (int)Math.Round((float)image.Height / f);
        ...
        public class Tuple<T1, T2> : Tuple<T1>
        {
            ...
            {
                int hashCode = Item1.GetHashCode();
