...
namespace Manina.Windows.Forms
{
    ...
    public class FileSystemAdaptor : ImageListViewItemAdaptor
    {
        ...
        public override string GetUniqueIdentifier(object key, Size size, UseEmbeddedThumbnails useEmbeddedThumbnails, bool useExifOrientation, bool useWIC)
        {
            ...
            if (Uri.TryCreate(filename, UriKind.RelativeOrAbsolute, out Uri uri)&&uri.IsAbsoluteUri&&!uri.IsFile)
            {
                string ip = uri.Host;
                string pathandquery = uri.PathAndQuery;
                List<string> querys = uri.Query.TrimStart('?').Split('&').ToList();
                foreach (var query in querys)
                {
                    if (!query.StartsWith("token")) 
                        continue;
                    if (querys.Count == 1)
                        pathandquery = pathandquery.Replace(uri.Query, "");
                    else
                    {
                        if (querys.IndexOf(query) == 0)
                            pathandquery = pathandquery.Replace(query, "");
                        else
                            pathandquery = pathandquery.Replace("&" + query, "");
                    }
                }

                filename = ip + "," + pathandquery;
            }
        ...
        public override Image GetThumbnail(object key, Size size, UseEmbeddedThumbnails useEmbeddedThumbnails, bool useExifOrientation, bool useWIC)
        {
            ...
            try
            {
                using (WebClient client = new WebClient())
                {
                    ...
                    Console.WriteLine("UriAdaptorDownImageUri:" + uri);
