    ...
    public class PersistentCache
    {
        ...
        public Stream Read(string id)
        {
            lock (lockObject)
            {
                ...
                string filename = Path.Combine(mDirectoryName, MakeKey(id));
                if (!File.Exists(filename))
                    return ms;
                try
                {
                    string base64text = File.ReadAllText(filename);
                    byte[] bitbytes = Convert.FromBase64String(base64text);
                    ms.Write(bitbytes, 0, bitbytes.Length);
                ...
                {
                    Console.WriteLine($"ReadImageCacheError,id:{id},filename:{filename}\n" + ex);
                    if (File.Exists(filename))
        ...
        public void Write(string id, Stream data)
        {
            lock (lockObject)
            {
                ...
                {
                    Console.WriteLine($"WriteImageCacheError,id:{id},filename:{filename}\n" + ex);
                    if (File.Exists(filename))
                        File.Delete(filename);
                    return;
                }

                mCurrentSize += bytes.Length;

                if (mCurrentSize > mSize / 2)
                    PurgeCache();
            }
        }
        ...
        public void Clear()
        {
            lock (lockObject)
            {
                ...
                {
                    Directory.Delete(mDirectoryName,true);
                    Directory.CreateDirectory(mDirectoryName);
                }
        ...
        private void CalculateSize()
        {
            lock (lockObject)
            {
                ...
                mCurrentSize = new DirectoryInfo(mDirectoryName).GetFiles().Sum(s => s.Length);
            }
        }
        ...
        private void PurgeCache()
        {
            lock (lockObject)
            {
                ...
                List<FileInfo> index = new DirectoryInfo(mDirectoryName).GetFiles().OrderBy(s => s.CreationTime)
                    .ToList();
