...
namespace Manina.Windows.Forms
{
    ...
    internal class ImageListViewLayoutManager
    {
        ...
        private void CalculateGrid()
        {
            ...
            mDisplayedRows = (int)Math.Floor((float)mItemAreaBounds.Height / (float)mItemSizeWithMargin.Height);
            mDisplayedCols = (int)Math.Floor((float)mItemAreaBounds.Width / (float)mItemSizeWithMargin.Width);
            ...
            if (mImageListView.View == View.Gallery)
            {
                ...
                mItemCols = (int)Math.Ceiling((float)mImageListView.Items.Count / (float)mDisplayedRows);
            }
            else
            {
                mItemCols = mDisplayedCols;
                mItemRows = (int)Math.Ceiling((float)mImageListView.Items.Count / (float)mDisplayedCols);
            }
        ...
        private void UpdateVisibleItems()
        {
            ...
            if (mImageListView.GroupsVisible)
            {
                ...
                foreach (ImageListViewGroup group in mImageListView.groups)
                {
                    ...
                    if (mFirstPartiallyVisible < 0)
                    {
                        if (mImageListView.View == View.Gallery)
                        {
                            mFirstPartiallyVisible = group.FirstItemIndex + (int)Math.Floor((float)(ItemAreaBounds.Left - group.itemBounds.Left) / (float)mItemSizeWithMargin.Width) * group.itemRows;
                            mFirstVisible = group.FirstItemIndex + (int)Math.Ceiling((float)(ItemAreaBounds.Left - group.itemBounds.Left) / (float)mItemSizeWithMargin.Width) * group.itemRows;
                        }
                        else
                        {
                            mFirstPartiallyVisible = group.FirstItemIndex + (int)Math.Floor((float)(ItemAreaBounds.Top - group.itemBounds.Top) / (float)mItemSizeWithMargin.Height) * group.itemCols;
                            mFirstVisible = group.FirstItemIndex + (int)Math.Ceiling((float)(ItemAreaBounds.Top - group.itemBounds.Top) / (float)mItemSizeWithMargin.Height) * group.itemCols;
                        }
                    }

                    if (mImageListView.View == View.Gallery)
                    {
                        mLastPartiallyVisible = group.FirstItemIndex + (int)Math.Ceiling((float)((ItemAreaBounds.Left - group.itemBounds.Left) + mItemAreaBounds.Width) / (float)mItemSizeWithMargin.Width) * group.itemRows - 1;
                        mLastVisible = group.FirstItemIndex + (int)Math.Floor((float)((ItemAreaBounds.Left - group.itemBounds.Left) + mItemAreaBounds.Width) / (float)mItemSizeWithMargin.Width) * group.itemRows - 1;
                    }
                    else
                    {
                        mLastPartiallyVisible = group.FirstItemIndex + (int)Math.Ceiling((float)((ItemAreaBounds.Top - group.itemBounds.Top) + mItemAreaBounds.Height) / (float)mItemSizeWithMargin.Height) * group.itemCols - 1;
                        mLastVisible = group.FirstItemIndex + (int)Math.Floor((float)((ItemAreaBounds.Top - group.itemBounds.Top) + mItemAreaBounds.Height) / (float)mItemSizeWithMargin.Height) * group.itemCols - 1;
                    }
                }
            }
            else
            {
                if (mImageListView.View == View.Gallery)
                {
                    mFirstPartiallyVisible = (int)Math.Floor((float)mImageListView.ViewOffset.X / (float)mItemSizeWithMargin.Width) * mDisplayedRows;
                    mLastPartiallyVisible = (int)Math.Ceiling((float)(mImageListView.ViewOffset.X + mItemAreaBounds.Width) / (float)mItemSizeWithMargin.Width) * mDisplayedRows - 1;
                    mFirstVisible = (int)Math.Ceiling((float)mImageListView.ViewOffset.X / (float)mItemSizeWithMargin.Width) * mDisplayedRows;
                    mLastVisible = (int)Math.Floor((float)(mImageListView.ViewOffset.X + mItemAreaBounds.Width) / (float)mItemSizeWithMargin.Width) * mDisplayedRows - 1;
                }
                else
                {
                    mFirstPartiallyVisible = (int)Math.Floor((float)mImageListView.ViewOffset.Y / (float)mItemSizeWithMargin.Height) * mDisplayedCols;
                    mLastPartiallyVisible = (int)Math.Ceiling((float)(mImageListView.ViewOffset.Y + mItemAreaBounds.Height) / (float)mItemSizeWithMargin.Height) * mDisplayedCols - 1;
                    mFirstVisible = (int)Math.Ceiling((float)mImageListView.ViewOffset.Y / (float)mItemSizeWithMargin.Height) * mDisplayedCols;
                    mLastVisible = (int)Math.Floor((float)(mImageListView.ViewOffset.Y + mItemAreaBounds.Height) / (float)mItemSizeWithMargin.Height) * mDisplayedCols - 1;
                }
            }
                ...
                mLastPartiallyVisible <= mImageListView.Items.Count - 1)
            {
                for (int i = mFirstPartiallyVisible; i <= mLastPartiallyVisible; i++)
                {
                    ...
                    if (mImageListView.Items[i].ThumbnailCacheState != CacheState.Cached)
                    {
                        mImageListView.thumbnailCache.Add(mImageListView.Items[i].Guid, mImageListView.Items[i].Adaptor, mImageListView.Items[i].VirtualItemKey,
                             mImageListView.ThumbnailSize, mImageListView.UseEmbeddedThumbnails, mImageListView.AutoRotateThumbnails,
                             (mImageListView.UseWIC == UseWIC.Auto || mImageListView.UseWIC == UseWIC.ThumbnailsOnly), 3);
                    }
                }
            }
        ...
        private void UpdateGroups()
        {
            ...
            foreach (ImageListViewGroup group in mImageListView.groups)
            {
                if (mImageListView.View == View.Gallery)
                {
                    ...
                    group.itemCols = (int)Math.Ceiling((float)group.ItemCount / (float)mDisplayedRows);
                ...
                else
                {
                    ...
                    group.itemRows = (int)Math.Ceiling((float)group.ItemCount / (float)mDisplayedCols);
