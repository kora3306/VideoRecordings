    ...
    internal static class ThumbnailExtractor
    {
        ...
        public static Image FromFile(string filename, Size size, UseEmbeddedThumbnails useEmbeddedThumbnails, bool useExifOrientation, bool useWIC)
        {
            ...
            if (useWIC)
            {
                ...
                try
                {
                    using (Stream stream = GetImageStream(filename))
                    {
                        // Performance vs image quality settings.
                        // Selecting BitmapCacheOption.None speeds up thumbnail generation of large images tremendously
                        // if the file contains no embedded thumbnail. The image quality is only slightly worse.
        ...
        private static Dictionary<string, byte[]> imageDatadic = new Dictionary<string, byte[]>();
        ...
        private static Stream GetImageStream(string filename)
        {
            ...
            {
                while (urlList.Contains(filename))
                    Thread.Sleep(10);
                lock (obj1)
                    urlList.Add(filename);
                try
                {
                    MemoryStream ms = GetUrlSteam(filename);
                    return ms;
                }
                catch(Exception ex)
                {
        ...
        private static MemoryStream GetUrlSteam(string url)
        {
            byte[] result = null;
            lock (obj)
            {
                if (imageDatadic.ContainsKey(url))
                    result = imageDatadic[url];
            }
            if (result == null)
            {
                try
                {
                    ...
                    using (WebClient web = new WebClient())
                    {
                        if (url.Contains("token"))
                        {
                            ...
                            string token = urlParams.FirstOrDefault(t => t.StartsWith("token"));
                            string cookies = token.Split('=').Last();
                            web.Headers.Add("Cookie", $"sessionid = {cookies}");
                        }

                        stopwatch.Start();
                        result = web.DownloadData(url);
                        stopwatch.Stop();
                    }

                    Console.WriteLine(
                        $"DownImage->{stopwatch.ElapsedMilliseconds}ms,uri:{url},thread:{Thread.CurrentThread.ManagedThreadId},source:FileAdaptor");
                }
                catch (Exception ex)
                {
                ...
                if (result != null && result.Length > 0)
                {
                    if (urlqueue.Count > 200)
                    {
                        lock (obj)
                        {
                            string removeUrl = urlqueue.Dequeue();
                            imageDatadic.Remove(removeUrl);
                        }
                    }
                    else
                    {
